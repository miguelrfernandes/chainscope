...
def test_router_prompt_whale_intent():
    ...
    assert "Do NOT treat the presence of a connected wallet address suffix as making that wallet the subject of every question" in ROUTER_SYSTEM_PROMPT


def test_portfolio_prompt_conditional_wallet():
    assert "ONLY call get_wallet_balances if the user's question is actually asking about their own wallet" in PORTFOLIO_PROMPT
